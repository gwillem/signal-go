...
**Outer-layer failure (ECDH with stale identity key):** The sender's identity is encrypted inside the outer sealed sender layer that failed — we don't know who sent the message. We cannot send a retry receipt. The sender must independently discover our new identity key and re-establish a session. The error is logged and the envelope is ACK'd. **Verified against Signal-Android:** `SealedSessionCipher.decrypt()` (libsignal `SealedSessionCipher.java:204-210`) wraps outer `DecryptToUsmc` failures as `InvalidMetadataMessageException`, and `MessageDecryptor.kt:266-271` handles this as `Result.Ignore` — no retry receipt.

**Recovery from outer-layer failures:** Signal-Android refreshes recipient identity keys through event-driven triggers, not a periodic poll. The relevant mechanisms are:

1. **`UntrustedIdentityException` on send** (`IndividualSendJob.java:215-224`): When sending to a recipient fails with a key mismatch, the sender immediately queues a `RetrieveProfileJob` to fetch the recipient's current identity key. This is the fastest recovery path, but only triggers when the *sender* tries to send to *us* via a non-sealed-sender path.
2. **Routine profile refresh** (`RetrieveProfileJob.kt`): Signal-Android periodically refreshes profiles for contacts, with a 12-hour interval and 5-minute debounce. This is the primary recovery path for outer sealed sender failures — the sender will eventually pick up our new identity key during a routine refresh. Expected recovery window: **up to ~12 hours**.
3. **Stale/mismatched device responses** (`SignalServiceMessageSender.java:2791-2838`): When the server returns 409 (mismatched devices) or 410 (stale devices) on send, the sender re-fetches pre-key bundles. This forces session re-establishment but only triggers on non-sealed-sender sends.
4. **Recipient sends a message**: If we send a message to the affected sender, they receive our current identity key as part of normal session establishment. This is the fastest manual recovery.

For outer sealed sender failures specifically, none of the fast triggers (1, 3) apply — the sender doesn't know the message failed. Recovery depends on the ~12 hour routine profile refresh (2) or the recipient initiating contact (4). This is an inherent limitation of sealed sender's privacy guarantees.
