...
### Signal Desktop Sync Message Timestamp Validation

**Status**: Fixed

**Problem**: When signal-go sent a group message, Signal Desktop received the sync message but threw an error:
```
Error: Timestamp 1770323044140 in DataMessage did not match envelope timestamp 1770323045211
```

**Root Cause**: Signal Desktop validates that the DataMessage timestamp matches the envelope timestamp. Our sync message used:
- DataMessage timestamp: original message time
- Envelope timestamp: new timestamp when sync was sent (generated by `encryptAndSend`)

**Fix**: Added `sendEncryptedMessageWithTimestamp()` function that accepts an explicit timestamp parameter. The sync message now passes the DataMessage timestamp so the envelope uses the same value.

**Location**: `internal/signalservice/retryreceipt.go` and `internal/signalservice/groupsender.go`

**Signal Desktop validation code** (`processDataMessage.preload.ts:477-481`):
```typescript
